...
export default function ImageGIG({navigation,route}){
  ...
  return(
    <View style={{height:'100%',width:'100%',backgroundColor:'#1e1e1e',justifyContent:'center',alignItems:'center'}} >
        ...
        <Text style={{fontFamily:'RubikMedium',fontSize:25,color:'white',textAlign:'center',alignSelf:'center',marginBottom:20}} >Ajouter des images</Text>

        <View style={{width:'80%',height:width*0.8,borderRadius:12,backgroundColor:'#2C2C2C',marginBottom:20,display:'flex',flexDirection:'row',flexWrap:'wrap'}} >
          {
            images.map((img)=>{
              return(
          ...
          {images.length<4?<TouchableOpacity onPress={pickImage} style={{width:(width*0.4-20),height:(width*0.4-20),margin:10,borderRadius:7,backgroundColor:'#999',alignItems:'center',justifyContent:'center'}} ><Text style={{fontSize:60,fontFamily:'RubikRegular',color:'white'}} >+</Text></TouchableOpacity>:null}
